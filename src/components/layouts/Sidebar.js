...
const Sidebar = () => {
  ...
  return (
    <div className="hidden lg:flex shadow-3xl flex-col w-60 h-full pb-8 bg-white text-gray-5 dark:bg-dark-white ">
        ...
        {sidebarItems.map(({ Icon, ...item }) => (
          <Link
            ...
            className={`flex items-center rounded-lg py-2.5 pl-5
             cursor-pointer duration-100 text-sm font-medium mt-3 dark:text-gray-4 ${pathname.includes(item.link)
                ? 'bg-primary-green text-white'
                : 'text-gray-5 hover:bg-gray-4/40  '
              }`}
          >
            <div>
              <Icon stroke={pathname.includes(item.link) ? 'white' : '#73797F'} />
            </div>
      ...
      <div role='button' onClick={handleLogout} className="w-fit mx-auto mt-auto flex items-center cursor-pointer ">
        <div>
